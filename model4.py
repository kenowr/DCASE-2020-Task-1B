...
augmented_features = np.load(feature_dir + 'augmented_train_features.npy',allow_pickle=True)
augmented_labels = to_categorical(np.load(feature_dir + 'augmented_train_labels.npy',allow_pickle=True))
...
model_path = 'models' + os.sep + 'model4' + os.sep
...
print('Generated Model 4, Subsystem 1. Training it now...')

## TRAIN INCEPTION MODEL
model_hist = trainp_model(model,train_features,train_labels,eval_features,eval_labels,
                          string = 'MODEL4_SUBSYSTEM1_', model_path = model_path, batch_size = batch_size)

for subsystem in range(2,7):
    ## GENERATE VGG MODEL (SUBSYSTEM 2-6)
    model = vgg_prunable(**pruning_params)
    print('Generated Model 4, Subsystem {}. Training it now...'.format(subsystem))

    ## TRAIN VGG MODEL
    model_hist = trainp_model(model,train_features,train_labels,eval_features,eval_labels,
                              string = 'MODEL4_SUBSYSTEM{}_'.format(subsystem), model_path = model_path, batch_size = batch_size)
    
## FIND BEST SUBSYSTEMS
best_filepaths = []
for subsystem in range(1,7):
    model_list = glob.glob('models' + os.sep + 'model4' + os.sep + 'weights' + os.sep + 'MODEL4_SUBSYSTEM{}_epoch-[3,4]*'.format(subsystem)) # Get models from epoch 300-400 only because they are pruned.
...
_ = make_predictions(test_features, best_filepaths, output_filepath = submission_dir + 'Ooi_NTU_task1b_4.output.csv', delimiter = '\t', newline = '\n')
...
print()
print('============================')
print(' FINAL ANALYSIS FOR MODEL 4 ')
print('============================')
print()
