...
model_path = 'models' + os.sep + 'model2' + os.sep
...
## GENERATE INCEPTION MODEL (MODEL 2)
model = inception_prunable(**pruning_params)
print('Generated Model 2. Training it now...')

## TRAIN INCEPTION MODEL
model_hist = trainp_model(model,train_features,train_labels,eval_features,eval_labels,
                          string = 'MODEL2_', model_path = model_path, batch_size = batch_size)

## FIND BEST MODEL
model_list = glob.glob('models' + os.sep + 'model2' + os.sep + 'weights' + os.sep + 'MODEL2_epoch-[3,4]*') # Get models from epoch 300-400 only because they are pruned.
_, best_filepath, _, _ = get_best_model(eval_features, eval_labels, model_list)
print('Best filepath is: ' + best_filepath)
...
_ = make_predictions(test_features, [best_filepath], output_filepath = submission_dir + 'Ooi_NTU_task1b_2.output.csv', delimiter = '\t', newline = '\n')
...
output_meta = make_predictions(eval_features, [best_filepath], save = False)
best_micro_acc, best_macro_acc = accs(np.array([row[-3:] for row in output_meta[1:]]), eval_labels)

## PRINT FINAL METRICS
print()
print('============================')
print(' FINAL ANALYSIS FOR MODEL 2 ')
print('============================')
print()
print('Model files used: ' + best_filepath)
